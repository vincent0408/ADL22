...
def parse_args():
    ...
    parser.add_argument("--do_predict", action="store_true", default=True, help="To do prediction on the question answering model")
    parser.add_argument(
        "--test_file", type=str, default="./pred_mc.json", help="A csv or a json file containing the Prediction data."
    )
    parser.add_argument(
        "--max_seq_length",
        type=int,
    ...
    parser.add_argument(
        ...
        default="./ckpt/question_answering",
        required=False,
    )
    parser.add_argument(
        "--config_name",
    ...
    parser.add_argument("--output_file", type=str, default="./pred_qa.csv", help="Where to store the final model.")
...
def main():
    ...
    # Log a few random samples from the predicting set:
    ...
    with open(args.output_file, 'w', encoding='utf-8') as f:
        f.write('id,answer\n')
        for pre in tqdm(prediction):
            pred = pre['prediction_text']
            if(',' in pred):
                pred = pred.replace(',', '')
            elif('《' in pred and '》' not in pred):
                pred = pred + '》'
            elif('》' in pred and '《' not in pred):
                pred = '《' + pred
            elif('「' in pred and '」' not in pred):
